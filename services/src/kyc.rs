...
impl Sep12Service {
    ...
    pub async fn put_customer(&self, request: CustomerRequest) -> Result<CustomerResponse, Sep12Error> {
         ...
         if let Some(binary_fields) = request.fields.binary_fields() {
             for (field_name, file) in binary_fields {
    ...
    pub async fn delete_customer(&self, account: &str, memo: Option<&str>) -> Result<(), Sep12Error> {
        let mut conn = establish_connection().map_err(|e| Sep12Error::DatabaseError(e.to_string()))?;
        ...
        diesel::delete(
            sep12_customers::table
                ...
                .filter(
                    memo.map(|m| sep12_customers::memo.eq(m))
                        .unwrap_or(sep12_customers::memo.is_null())
                )
        )
        .execute(&mut conn)
        .map_err(|e| Sep12Error::DatabaseError(e.to_string()))?;
    
        // Then delete from remote KYC server
        let jwt = self.auth.authenticate(account, &self.keypair).await?;
        ...
        if let Some(memo) = memo {
            request = request.query(&[("memo", memo)]);
        }
    ...
    pub async fn submit_verification(
        ...
        pub async fn upload_file(&self, upload: FileUpload) -> Result<FileResponse, Sep12Error> {
            ...
            let file: Sep12CustomerFile = diesel::insert_into(sep12_customer_files::table)
                .values(&new_file)
                .get_result(&mut conn)
                .map_err(|e| Sep12Error::DatabaseError(e.to_string()))?;
