...
use stellar_base::{KeyPair, Network, Operation, transaction::TransactionEnvelope};
...
pub async fn authenticate(
    ...
    let anchor_config = anchor_service
        .get_anchor_config_details(slug)
        .await
    ...
    let home_domain: Option<&str> = anchor_config
        .documentation
    ...
    let account_id_string = keypair.public_key().to_string();
...
fn verify_challenge_structure(
    ...
    if *transaction.sequence() != 0 {
        return Err(AuthError::ChallengeError(
            "Sequence number must be 0".into(),
        ));
    }
    ...
    let signature_data = transaction
    ...
    if operations.is_empty() {
        return Err(AuthError::ChallengeError(
            "No operations in challenge".into(),
        ));
    }
        ...
        if let Some(source) = op.source_account() {
            if source.account_id().to_string() != client_account {
                return Err(AuthError::ChallengeError(format!(
                    "First operation source must be {}",
                    client_account
                )));
            }
        } else {
